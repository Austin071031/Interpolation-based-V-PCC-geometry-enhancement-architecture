...
iters_path = '/home/jupyter-austin2/pc_dataset/Complement_dataset/coordenhanced_ply_interpolation/ply_qp40_cube70/interpolate_30/val/noisyinput/'
rec_path = '/home/jupyter-austin2/pc_dataset/combine_ply/gqe_output/gqe_color/qp40/coordenhanced_interpolation_noisyinput/'
if not os.path.exists(rec_path):
    os.makedirs(rec_path)

# for iteration in os.listdir(iters_path):
#     blocks_path = os.path.join(iters_path, iteration)
#     print(blocks_path)
filelist = os.listdir(iters_path)
#print(filelist)



filelist.sort(key=lambda y : int(y.split('.')[0].split('_')[-2]))
...
for i in range (len(filelist) + 1):
    ...
    if(i == len(filelist) or (filelist[i].split('_')[-2] != current_frame)):
        print('Reconstruct Frame: ' + str(filelist[i-1].split('_')[-2]))
        end = i - 1
        suffix = filelist[end].split('.')[-1]
        filename = "_".join(filelist[end].split('_')[:-1])
#         rec_iter = rec_path + iteration + '/' 
        if not os.path.exists(rec_path):
            os.makedirs(rec_path)
        ...
        with open(rec_file, 'w') as fw:
            for j in range(start, end+1):
                ...
                file_path = os.path.join(iters_path, filelist[j])
                print(file_path)
    ...
    if(i != len(filelist)):
        file_path = os.path.join(iters_path, filelist[i])
        print(filelist[i])
