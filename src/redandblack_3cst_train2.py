...
from data2 import make_data_loader
...
def parse_args():
    ...
    parser.add_argument("--dataset_8i", default='/home/jupyter-austin2/pc_dataset/self_train/qp47/testredandblack/ply_block_coords_nooptimized/60frames/train/noisyinput/')
    parser.add_argument("--dataset_8i_GT", default='/home/jupyter-austin2/pc_dataset/self_train/qp47/testredandblack/ply_block_coords_nooptimized/60frames/train/ground/')
    parser.add_argument("--dataset_8i_val", default='/home/jupyter-austin2/pc_dataset/self_train/qp47/testredandblack/ply_block_coords_nooptimized/30frames/val/noisyinput/')
    parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/self_train/qp47/testredandblack/ply_block_coords_nooptimized/30frames/val/ground/')
...
#     parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/xyz(fixed)_rgb(noise)/test_val_gt/')
    ...
    parser.add_argument("--prefix", type=str, default='ME_60framestrain_COnoFEU_testredandblack47noloot_30000_noisymodel_xyzunique_y_batchsize8_lr0.001_lrstep12000', help="prefix of checkpoints/logger, etc.")
    ...
    parser.add_argument("--train_loss_loc", default='/home/jupyter-austin2/train_loss/')
...
#     crit = torch.nn.BCEWithLogitsLoss()
    ...
    for i in range(len(test_iter)):
        try:
            coords, feats, coords_T, feats_T, filedir = test_iter.next()
        except StopIteration:
            test_iter = iter(test_dataloader)
            coords, feats, coords_T, feats_T, filedir = test_iter.next()
### yuv
#         feat_yuv = np.zeros(feats.shape)
#         feat_yuv[:, 0] = 0.2126*feats[:, 0] + 0.7152*feats[:, 1] + 0.0722*feats[:, 2]

#         feat_yuv[:, 1] = -0.1146*feats[:, 0] - 0.3854*feats[:, 1] + 0.5000*feats[:, 2]

#         feat_yuv[:, 2] = 0.5000*feats[:, 0] - 0.4542*feats[:, 1] - 0.0458*feats[:, 2]
        
#         feat_yuv_gt = np.zeros(feats.shape)
#         feat_yuv_gt[:, 0] = 0.2126*coords_T[:, 0] + 0.7152*coords_T[:, 1] + 0.0722*coords_T[:, 2]
#         feat_yuv_gt[:, 1] = -0.1146*coords_T[:, 0] - 0.3854*coords_T[:, 1] + 0.5000*coords_T[:, 2]

#         feat_yuv_gt[:, 2] = 0.5000*coords_T[:, 0] - 0.4542*coords_T[:, 1] - 0.0458*coords_T[:, 2]

        ### yuv       
#         feat_yuv_tensor = torch.from_numpy(feat_yuv).to(torch.float32)
#         feat_yuv_gt_tensor = torch.from_numpy(feat_yuv_gt).to(torch.float32)
        
        coords_In = coords[:, 1:].to(torch.float32)
        x = ME.SparseTensor(features=coords_In, coordinates=coords, device=device, requires_grad=True)
### yuv
...
#         coords_gt = ME.SparseTensor(features=feats_T.cuda(), coordinates=coords_T, device=device, requires_grad=True)
...
#         loss = crit(out_cls.F.squeeze(), target.type(out_cls.F.dtype).to(device))
        loss = crit(out_cls.F, coords_T[:, 1:].type(torch.float).to(device))
...
####	restruct ply file                
        out_coords = out_cls.F.cpu().detach().numpy()
        noisy_pc = o3d.io.read_point_cloud(filedir[0])
        pcd_tree = o3d.geometry.KDTreeFlann(noisy_pc)
        rgb = np.asarray(noisy_pc.colors)
        rgb2 = np.zeros((len(out_coords), 3))
        for c,n in enumerate(out_coords):
            [_, idx,_ ] = pcd_tree.search_knn_vector_3d(n, 1)
            rgb2[c,:] = rgb[idx, :]
        ### yuv
#         out_yuv = out_cls.F.cpu().detach().numpy()
#         out_y = out_cls.F.cpu().detach().numpy()[:, 0]
#         out_u = out_cls.F.cpu().detach().numpy()[:, 1]
#         out_v = out_cls.F.cpu().detach().numpy()[:, 2]
...
#         for i in range(len(train_iter)):
        try:
            coords, feats, coords_T, feats_T, filedir = train_iter.next()
        except StopIteration:
            train_iter = iter(train_dataloader)
            coords, feats, coords_T, feats_T, filedir = train_iter.next()
...
#         coords_gt = ME.SparseTensor(features=feats_T, coordinates=coords_T, device=device, requires_grad=True)
