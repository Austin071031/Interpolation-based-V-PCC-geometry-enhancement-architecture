...
def parse_args():
    ...
    parser.add_argument("--dataset_8i", default='/home/jupyter-austin2/pc_dataset/MIR_Dataset/ply_cube/ply_qp36_cube40_overlap10/train/noisyinput_nosmoll/')
    parser.add_argument("--dataset_8i_GT", default='/home/jupyter-austin2/pc_dataset/MIR_Dataset/ply_cube/ply_qp36_cube40_overlap10/train/ground/')
    parser.add_argument("--dataset_8i_val", default='/home/jupyter-austin2/pc_dataset/MIR_Dataset/ply_cube/ply_qp36_cube40_overlap10/val/noisyinput_nosmoll/')
    parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/MIR_Dataset/ply_cube/ply_qp36_cube40_overlap10/val/ground/')
...
#     parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/xyz(fixed)_rgb(noise)/test_val_gt/')
    ...
    parser.add_argument("--global_step", type=int, default=int(500))
    ...
    parser.add_argument("--prefix", type=str, default='ME_colorMUNET_qp36_cube70_batchsize2_lr0001_test3', help="prefix of checkpoints/logger, etc.")
...
#     crit = torch.nn.BCEWithLogitsLoss()
    ...
    for i in range(len(test_iter)):
        try:
            coords, coords_feats, feats, coords_T, feats_T, filedir = test_iter.next()
        except StopIteration:
            test_iter = iter(test_dataloader)
            coords, coords_feats, feats, coords_T, feats_T, filedir = test_iter.next()
            
        print(filedir)
### coords transform relative coords
#         relative_coords, center = transform_relative(feats)
#         relative_coordsT, centerT = transform_relative(coords_T)
### coords normalize        
#         ### coords normalize
...
# #         # insert one column of scalar to input 
# #         scalar = np.random.rand(coords_norm.shape[0])
# #         np.insert(coords_norm, 0, scalar, axis=1)
...
### yuv
        ...
        y_gt = 0.2126*feats_T[:, 0] + 0.7152*feats_T[:, 1] + 0.0722*feats_T[:, 2]
        ...
        x = ME.SparseTensor(features=y_re.cuda(), coordinates=coords.cuda())
#         sp_coordsT_norm = relative_coordsT.cuda()
        
#         coords_gt = ME.SparseTensor(features=coordsT_norm.cuda(), coordinates=coords_T, device=device, requires_grad=True)
        ...
        noisy_xyz = out_cls.C[:, 1:]
        loss = color_loss.get_color_mse_loss(out_cls.F, noisy_xyz, y_gt_re.cuda(), coords_T.cuda())
...
####	restruct ply file                
#         coords_rec = inverse_transform_relative(out_cls.F.cpu(), center)
#         coords_rec = coords_rec.numpy()
#         coords_rec = min_max_unnormalize(scaler, out_cls.F, x.F)
#         out_coords_unnorm = unnormalize(out_coords.unsqueeze(0), 0.5, 1, max_value)
      

        feats = feats.numpy()
        coords = out_cls.C[:, 1:].cpu().detach().numpy()
        ### yuv
...
#         # yuv --> rgb   
# #         out_y = out_cls.F
        out_r = out_y + 1.402*out_v
        out_g = out_y - 0.344*out_u - 0.792*out_v
        out_b = out_y + 1.772*out_u
...
#         print(recfile)
        ...
        with open(recfile, 'w') as f:
            ...
            f.write('element vertex ' + str(len(coords)) + '\n')
...
#         for j in range(len(train_iter)):
        try:
            coords, coords_feats, feats, coords_T, feats_T, filedir = train_iter.next()
        except StopIteration:
            train_iter = iter(train_dataloader)
            coords, coords_feats, feats, coords_T, feats_T, filedir = train_iter.next()
...
### coords transform relative coords
#         relative_coords, _ = transform_relative(feats)
#         relative_coordsT, _ = transform_relative(coords_T)
### coords normalize
#         coords_copy, coordsT_copy = feats.float().numpy(), coords_T.float().numpy()

#         #normalize
#         scaler, coords_norm = min_max_normalize(coords_copy, 64)
#         scaler, coordsT_norm = min_max_normalize(coordsT_copy, 64)
...
### yuv
        ...
        x = ME.SparseTensor(features=y_re, coordinates=coords, device=device, requires_grad=True)

#         sp_coordsT_norm = relative_coordsT.cuda()
#         coords_gt = ME.SparseTensor(features=coordsT_norm, coordinates=coords_T, device=device, requires_grad=True)

        if x.__len__() >= 1e10:
            logger.info(f'\n\n\n======= larger than 1e10 ======: {x.__len__()}\n\n\n')
            continue
        # Forward.
        _, out_cls = model(x, coords_T=coords, device=device, prune=False)
