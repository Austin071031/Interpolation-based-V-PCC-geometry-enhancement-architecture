...
# from PyTorchEMDmaster.test_emd_loss import EMD_Loss
...
def parse_args():
    ...
    parser.add_argument("--dataset_8i", default='/home/jupyter-austin2/pc_dataset/Complement_dataset/ply_interpolation/ply_qp40_cube100/interpolate_50/train/noisyinput/')
    parser.add_argument("--dataset_8i_GT", default='/home/jupyter-austin2/pc_dataset/Complement_dataset/ply_cube/ply_qp40_cube100/train/ground_lost/')
    parser.add_argument("--dataset_8i_val", default='/home/jupyter-austin2/pc_dataset/Complement_dataset/ply_interpolation/ply_qp40_cube100/interpolate_50/val/noisyinput/')
    parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/Complement_dataset/ply_cube/ply_qp40_cube100/val/ground_lost/')
...
#     parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/xyz(fixed)_rgb(noise)/test_val_gt/')
    ...
    parser.add_argument("--clip_value", type=float, default=1, help='max value for gradient clip, close if 0')

    parser.add_argument("--logdir", type=str, default='logs', help="logger direction.")
    parser.add_argument("--ckptdir", type=str, default='/home/jupyter-austin2/pc_dateset/denoising_model/', help="ckpts direction.")
    parser.add_argument("--prefix", type=str, default='MEtest8i_qp40_interpolate50_cube100_lr0001_batchsize4', help="prefix of checkpoints/logger, etc.")
    ...
    parser.add_argument("--lr_step", type=int, default=1200, help="step for adjusting lr_scheduler.")
...
#         print(pad_coord_feats)
#         print(pad_coords) 

#         num_coord_feats = coord_feats.cpu().detach().numpy()
#         min_max_scaler, norm_coords = min_max_normalize(num_coord_feats, args.normalize)
...
#         tensor_coords = torch.from_numpy(coords)
        x = ME.SparseTensor(features=coord_feats.float().cuda(), coordinates=coords.cuda())
        x_feats = ME.SparseTensor(features=feats.cuda(), coordinates=coords.int().cuda())
    
#         tensor_coords_T = torch.from_numpy(coords_T)
        x_gt =  ME.SparseTensor(features=coords_T.float().cuda(), coordinates=coords_T.int().cuda())

        # Forward.
        with torch.no_grad():
            _, out_cls = model(x, coords_T=coords_T, device=device, prune=True)

            
        # Loss calculation
#         coords_gt = norm_coords_T_tensor.unsqueeze(0).to(device)
...
#         loss = torch.tensor(np.sqrt(dis.detach().cpu().numpy()).mean()).to(device)
        #loss = pd_loss.get_mse_loss2(pred, coords_gt, 0.2)
        loss = pd_loss.get_mse_loss(out_cls.F.unsqueeze(0), coords_T.to(device).unsqueeze(0), 0.2)
#         loss = get_loss(pred, coords_gt)
     #   loss = mse_loss(coords_T, out_cls.F)
#         loss = mse_loss(coords_T.to(device), out_cls.F)
#        loss = crit(out_cls.F,coords_T.to(device))
...
####	restruct ply file
        ...
        feats_num = x_feats.F.cpu().detach().numpy()
#         generated_coords = min_max_unnormalize(min_max_scaler, generated_norm_coords)
        
# #         if len_coord <= args.padding_cube:
# #             generated_coords = generated_coords[:len_coord]
# #         else:
# #             generated_coords = generated_coords
        generated_coords_tensor = torch.from_numpy(generated_norm_coords)
    
#         k_indices, k_dis = my_knn_query(1, generated_coords_tensor.unsqueeze(0).cuda(), coord_feats.unsqueeze(0).cuda(), device)
        
        align_gen_knn = KNN(k=1, transpose_mode=True)
        _, idx = align_gen_knn(coord_feats.float().unsqueeze(0).cuda(), generated_coords_tensor.float().unsqueeze(0).cuda())
        generated_feats_tensor = torch.zeros(generated_norm_coords.shape)

        for k in range(generated_coords_tensor.shape[0]):
            generated_feats_tensor[k] = feats[idx[0][k][0]]
        feats_num = generated_feats_tensor.numpy()

#         generated_coords_tensor = torch.from_numpy(generated_coords)
        
#         color_align_KNN = KNN(k=1, transpose_mode=True)
#         _, aligned_color_idx = color_align_KNN(coord_feats.to(device).unsqueeze(0), generated_coords_tensor.to(device).unsqueeze(0)) 
...
#         pc_rec = np.vstack((noisyinput_pc, generated_pc))
        ...
        print(out_path)
#         gen_out_path = os.path.join(args.save_loc, 'gen_' + args.prefix, str(step))
        if not os.path.exists(out_path):
            os.makedirs(out_path)	
#         if not os.path.exists(gen_out_path):
#             os.makedirs(gen_out_path)
        filedir_split = filedir[0].split('/')
        recfile = os.path.join(out_path, filedir_split[-1])
#         genfile = os.path.join(gen_out_path, filedir_split[-1])
...
#             f.write('element vertex ' + str(len(generated_pc)) + '\n')
...
#             f.write('end_header\n')
#             for i in range(len(generated_pc)):
#                 x = generated_pc[i][0]
#                 y = generated_pc[i][1]
#                 z = generated_pc[i][2]
#                 r = generated_pc[i][3]
...
#                 f.write(line + '\n')  
        ...
        with open(recfile, 'w') as f:
            ...
            f.write('comment frame_to_world_translation -39.1599 3.75652 -46.6228\n')
            f.write('comment width 1023\n')
            f.write('element vertex ' + str(len(generated_norm_coords)) + '\n')
            f.write('property float x\n')
            f.write('property float y\n')
            f.write('property float z\n')
            ...
            f.write('end_header\n')
            for h in range(len(generated_norm_coords)):
                x = generated_norm_coords[h][0]
                y = generated_norm_coords[h][1]
                z = generated_norm_coords[h][2]
                r = int(feats_num[h][0])
                g = int(feats_num[h][1])
                b = int(feats_num[h][2])
                line = str(x) + ' ' + str(y) + ' ' + str(z) + ' ' + str(r) + ' ' + str(g) + ' ' + str(b)
    ...
    current_frame = filelist[0].split('_')[-1]
    start = 0
    end = 0
    tot_length = 0
...
#     transformer = Transformer_Model(args, device = device)
    # save train loss
    train_result = []
    # Optimizer.
...
#         q, coarse_point_cloud = transformer(coord_feats.unsqueeze(0),device = device)
#         print("q:")
#         print(q)
...
#         min_max_scaler, norm_coords = min_max_normalize(coord_feats, args.normalize)
#         norm_coords_tensor = torch.from_numpy(norm_coords)
#         _, norm_coords_T = min_max_normalize(coords_T.cpu().detach().numpy(), args.normalize)
#         norm_coords_T_tensor = torch.from_numpy(norm_coords_T)

#         tensor_coords = torch.from_numpy(coords)
        x = ME.SparseTensor(features=coord_feats.cuda(), coordinates=coords.int().cuda())

#         num_coord_feats = coord_feats.cpu().detach().numpy()
            
#         _, norm_coords = min_max_normalize(num_coord_feats, args.normalize)
#         norm_coords_tensor = torch.from_numpy(norm_coords)
...
#         print(coord_feats)
#         print(coord_feats.shape)
    
#         tensor_coords_T = torch.from_numpy(coords_T)
#         x_gt = ME.SparseTensor(features=coords_T.float(), coordinates=coords_T.int(), device=device, requires_grad=True)
#         print(x.F)
#         print(x.F.shape)
        if x.__len__() >= 1e10:
        ...
        # Loss calculation
        loss = pd_loss.get_mse_loss(out_cls.F.unsqueeze(0), coords_T.to(device).unsqueeze(0), 0.2)
#        loss = crit(coords_T.to(device),out_cls.F)
...
#             all_metrics = np.zeros((1, 3))
        ...
        if i % (args.test_step) == 0:
            ...
            torch.save({'step': i, 'model': model.state_dict(),
                        ...
                        }, os.path.join(ckptdir, str(i) + '.pth'))
