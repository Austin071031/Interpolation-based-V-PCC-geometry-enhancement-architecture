...
#     parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/xyz(fixed)_rgb(noise)/test_val_gt/')
    ...
    parser.add_argument("--prefix", type=str, default='ME_colorMUNET_qp48_cube70_in30_batchsize2_lr0001', help="prefix of checkpoints/logger, etc.")
...
### yuv
        ...
        u = -0.1146*feats[:, 0] - 0.3854*feats[:, 1] + 0.5000*feats[:, 2] + 128
        u_re = u.reshape(-1, 1)
        
        v = 0.5000*feats[:, 0] - 0.4542*feats[:, 1] - 0.0458*feats[:, 2] + 128
        v_re = v.reshape(-1, 1)
...
### yuv
        ...
        coords_gt = ME.SparseTensor(features=y_gt_re.cuda(), coordinates=coords_T, device=device, requires_grad=True)
...
#         loss = crit(output, sp_coordsT_norm.type(torch.float).to(device))
        loss = crit.get_color_mse_loss(out_cls.F, out_cls.C[:, 1:], coords_gt.F, coords_gt.C[:, 1:])
...
####	restruct ply file                
        out_coords = out_cls.C[:, 1:].cpu().numpy()
#         coords_rec = min_max_unnormalize(scaler, out_coords)
...
#         # yuv --> rgb   
        out_y = out_cls.F[:, 0]
        out_u = u_tensor.F[:, 0] - 128
        out_v = v_tensor.F[:, 0] - 128
        
        out_r = out_y + 1.57480*out_v
        out_g = out_y - 0.18733*out_u - 0.46813*out_v
        out_b = out_y + 1.85563*out_u
...
#         out_b = out_y + 1.85563*out_u
        ...
        out_g = out_g.cpu().numpy()
        out_b = out_b.cpu().numpy()
        
#         feats_noisy = feats.numpy()
...
#         sp_coordsT_norm = coordsT_norm_tensor.cuda()
        coords_gt = ME.SparseTensor(features=y_gt_re, coordinates=coords_T, device=device, requires_grad=True)
