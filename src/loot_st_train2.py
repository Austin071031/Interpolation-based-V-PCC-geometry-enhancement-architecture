...
def parse_args():
    ...
    parser.add_argument("--dataset_8i", default='/home/jupyter-austin2/pc_dataset/self_train/qp37/testloot/ply_block/60frames/train/noisyinput/')
    parser.add_argument("--dataset_8i_GT", default='/home/jupyter-austin2/pc_dataset/self_train/qp37/testloot/ply_block/60frames/train/ground/')
    parser.add_argument("--dataset_8i_val", default='/home/jupyter-austin2/pc_dataset/self_train/qp37/testloot/ply_block/30frames/val/noisyinput/')
    parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/self_train/qp37/testloot/ply_block/30frames/val/ground')
...
#     parser.add_argument("--dataset_8i_val_GT", default='/home/jupyter-austin2/pc_dataset/xyz(fixed)_rgb(noise)/test_val_gt/')
    ...
    parser.add_argument("--prefix", type=str, default='ME_60framestrain_testloot37_30000_noisymodel_xyzunique_y_batchsize8_lr0.001_lrstep12000', help="prefix of checkpoints/logger, etc.")
...
#     crit = torch.nn.BCEWithLogitsLoss()
    ...
    for i in range(len(test_iter)):
        try:
            coords, feats, feats_T, filedir = test_iter.next()
...
#         v_tensor = ME.SparseTensor(features=v.cuda(), coordinates=coords.cuda())
        ...
        # Forward.
        with torch.no_grad():
            out, out_cls = model(x, coords_T=coords, device=device, prune=True)
...
#         for i in range(len(train_iter)):
        try:
            coords, feats, feats_T, filedir = train_iter.next()
        except StopIteration:
            train_iter = iter(train_dataloader)
            coords, feats, feats_T, filedir = train_iter.next()
