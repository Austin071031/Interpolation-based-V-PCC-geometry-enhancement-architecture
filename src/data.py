...
#         noisy_filename = os.path.basename(files[0])
#         filename = noisy_filename.split('.')[0]
#         ply_postfix = noisy_filename.split('.')[-1]
#         ground_filename = '_'.join(filename.split('_')[:-2]) + '.' + ply_postfix
#         ground_file_temp = os.path.join(GT_folder, ground_filename)
#         coords_T_keep = loadply(ground_file_temp, 'None')
...
#                 post_fix = name.split('.')[-1]
#                 temp = name.split('.')[0]
#                 ground = '_'.join(temp.split('_')[:-2]) + '.' + post_fix
                gt_file = os.path.join(GT_folder, name)
#                 if ground == ground_filename:
#                     if f.endswith('.ply'):
#                         coords, feats = loadply(f, feature_format)
#                         coords_T = coords_T_keep
#                 else:
#                     ground_filename = ground
#                     gt_file = os.path.join(GT_folder, ground_filename)
...
#                         coords_T_keep = coords_T
                ...
                if f.endswith('.h5'):
                    ...
                    coords, feats = loadply(f, feature_format)
                    coords_T, feats_T = loadply(gt_file, feature_format)
...
def collate_pointcloud_fn(list_data):
    ...
    coords_feats_batch = torch.from_numpy(np.vstack(coords)).float()
...
#     coords_T_batch = ME.utils.batched_coordinates(coords_T)
    ...
    return coords_batch, coords_feats_batch, feats_batch, coords_T_batch, feats_T_batch, filedir
